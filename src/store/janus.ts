...
class JanusStore {
    ...
    Janus.init({
      ...
      callback: () => {
        this.janus = new Janus({
          server: ["wss://janus-gateway.koreansimple.com:8189/janus", "https://janus-gateway.koreansimple.com/janus"],
          success: () => {
            this.attach(callback);
          },
          error: () => {},
  ...
  toggleAudio() {
    ...
    let muted = this.mainHandle?.isAudioMuted()
    if(muted) {
      this.mainHandle?.unmuteAudio()
    } else {
      this.mainHandle?.muteAudio()
    }
  }
  ...
  toggleVideo() {
    ...
    let muted = this.mainHandle?.isVideoMuted()
    if(muted) {
      this.mainHandle?.unmuteVideo()
    } else {
      this.mainHandle?.muteVideo()
    }
  }
}
